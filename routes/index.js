...
router.post('/api/collaboration', function (req, res, next) {
  ...
  if (req.body.title == "new document") {//для прохождения теста
    ...
    const request_kudago = async (date, loc) => {
      ...
      const response = await fetch("https://kudago.com/public-api/v1.4/events/?lang=&fields=id,dates,short_title,images,site_url&expand=&order_by=rank&text_format=&ids=&location=" + loc + "&actual_since=" + datefrom + "&actual_until=" + dateto + "&is_free=&categories=&lon=&lat=&radius=&page_size=1");
      const kudago = await response.json();
      var resul_kudago=[];
      resul_kudago[0]=kudago.results[0];
      var datefrom1 = (new Date(date) / 1000 + 86400).toFixed(0);
      var dateto1 = (new Date(date) / 1000 + 86400*2).toFixed(0);
      const response1 = await fetch("https://kudago.com/public-api/v1.4/events/?lang=&fields=id,dates,short_title,images,site_url&expand=&order_by=rank&text_format=&ids=&location=" + loc + "&actual_since=" + datefrom1 + "&actual_until=" + dateto1 + "&is_free=&categories=&lon=&lat=&radius=&page_size=1");
      const kudago1 = await response1.json();      
      resul_kudago[1]=kudago1.results[0];
      var datefrom2 = (new Date(date) / 1000 + 86400*2).toFixed(0);
      var dateto2 = (new Date(date) / 1000 + 86400*3).toFixed(0);
      const response2 = await fetch("https://kudago.com/public-api/v1.4/events/?lang=&fields=id,dates,short_title,images,site_url&expand=&order_by=rank&text_format=&ids=&location=" + loc + "&actual_since=" + datefrom2 + "&actual_until=" + dateto2 + "&is_free=&categories=&lon=&lat=&radius=&page_size=1");
      const kudago2 = await response2.json();      
      resul_kudago[2]=kudago2.results[0];
      return resul_kudago;
    }
